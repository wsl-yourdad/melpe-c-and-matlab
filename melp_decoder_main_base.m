...
function [sp_buf, state,params] = melpe_s(bit_str, state, cb, disp_filter)
    ...
    if params.mode == 1
        if isfield(params.lsf_indices, 'base')
        lsf_debug_tag = sprintf('M1/Base:%-3d', params.lsf_indices.base);
    else
        lsf_debug_tag = sprintf('VVV/S1 :%-3d', params.lsf_indices.base_s1);
    end
    else
    ...
    % 基音 (末帧基音)
   % [替换开始] Pitch 解码
    % 使用 VQ 函数，传入 uv_encoded (0-7) 才能正确还原 11-bit
    [pitch_vec_lin, uv_flags_real] = decode_pitch_vq_1200(params.uv_encoded, params.pitch_idx, cb);
    
    % 为了兼容后面代码对 final_pitch 的引用 (虽然逻辑上最好直接用 pitch_vec_lin)
    final_pitch = pitch_vec_lin(3); 
    % [替换结束]
    ...
    for f = 1:3
        ...
        % [替换开始] 直接使用 VQ 解码出的真实基音
        % [增强版] 基音获取与 UV 强制对齐
        p_val = pitch_vec_lin(f);
        uv_val = uv_flags_real(f);
        
        % 1. 如果是清音，强制 Pitch 归零，且 BPVC 归零
        if uv_val == 0
            curr_pitch = 0;
            curr_bpvc = zeros(5, 1); % 强制清音时 BPVC 也清零
        else
            % 2. 如果是浊音但 Pitch 异常小，给予最小保护或转清音
            if p_val < 20
                curr_pitch = 0; % 视为清音
            else
                curr_pitch = p_val;
        ...
        if f == 2 && params.jitter_idx == 1 && curr_pitch > 0
            curr_pitch = curr_pitch * 0.85; % 模拟抖动
        end
...
% =========================================================================
% 81-Bit Unpacker (极性修正版)
% 核心修正: 
%   1. 将 UV 比特取反 (1-b)，适配 melpe_s 中 "1=Voiced" 的判断逻辑
%   2. 保持 LSF indices 字段名一致
% =========================================================================
    ...
    % 2. UV (关键修正)
    % C标准: 0=Voiced, 1=Unvoiced
    % MATLAB逻辑: 1=Voiced, 0=Unvoiced
    % ---> 必须执行 (1 - bit) 反转！
    raw_uv_bits = [str2double(b_str(2)), str2double(b_str(3)), str2double(b_str(4))];
    p.uv = 1 - raw_uv_bits; % [0 0 1] -> [1 1 0] (VVU)
    
    % Encoded 整数 (用于模式判断，保持原始值用于查表)
    p.uv_encoded = bin2dec(b_str(2:4)); 
    
    % 3. Parity & Pitch
    p.parity = b_str(5);
    p.pitch_idx = bin2dec(b_str(6:14));
    
    % 4. LSF Raw Bits (42 bits)
    lsf_raw_bits = b_str(15:56);
    
    % 5. Other Parameters
    p.gain_idx   = bin2dec(b_str(57:66)) + 1;
    p.bpvc_idx   = bin2dec(b_str(67:72)); 
    p.fs_idx     = bin2dec(b_str(73:80)) + 1;
    p.jitter_idx = bin2dec(b_str(81));

    % === LSF 模式解析 ===
    % 001 (Decimal 1) -> Mixed Mode (VVU)
    if p.uv_encoded == 1
        p.mode = 1; 
        p.is_case_001 = true;
        
        ptr = 1;
        p.lsf_indices.base     = bin2dec(lsf_raw_bits(ptr : ptr+8)) + 1; ptr=ptr+9;
        p.lsf_indices.base_idx = p.lsf_indices.base; % 兼容别名
        
        p.lsf_indices.int_idx  = bin2dec(lsf_raw_bits(ptr : ptr+3)) + 1; ptr=ptr+4;
        p.lsf_indices.res_s1   = bin2dec(lsf_raw_bits(ptr : ptr+7)) + 1; ptr=ptr+8;
        p.lsf_indices.res_s2   = bin2dec(lsf_raw_bits(ptr : ptr+5)) + 1; ptr=ptr+6;
        p.lsf_indices.res_s3   = bin2dec(lsf_raw_bits(ptr : ptr+5)) + 1; ptr=ptr+6;
        p.lsf_indices.res_s4   = bin2dec(lsf_raw_bits(ptr : ptr+5)) + 1; ptr=ptr+6;
        
    elseif ismember(p.uv_encoded, [3, 5, 6, 7])
        % VVV Mode
        p.mode = 1; 
        p.is_case_001 = false;
        
        ptr = 1;
        p.lsf_indices.base_s1  = bin2dec(lsf_raw_bits(ptr : ptr+7)) + 1; ptr=ptr+8;
        p.lsf_indices.base_s2  = bin2dec(lsf_raw_bits(ptr : ptr+5)) + 1; ptr=ptr+6;
        p.lsf_indices.base_s3  = bin2dec(lsf_raw_bits(ptr : ptr+4)) + 1; ptr=ptr+5;
        p.lsf_indices.base_s4  = bin2dec(lsf_raw_bits(ptr : ptr+4)) + 1; ptr=ptr+5;
        
        % 给 base 一个值防止 debug 打印报错
        p.lsf_indices.base     = p.lsf_indices.base_s1; 
        ...
        p.lsf_indices.f1_s1 = bin2dec(lsf_raw_bits(1:8)) + 1;
        p.lsf_indices.f1_s2 = bin2dec(lsf_raw_bits(9:14)) + 1;
        p.lsf_indices.f2_s1 = bin2dec(lsf_raw_bits(15:22)) + 1;
        p.lsf_indices.f2_s2 = bin2dec(lsf_raw_bits(23:28)) + 1;
        p.lsf_indices.f3_s1 = bin2dec(lsf_raw_bits(29:36)) + 1;
        p.lsf_indices.f3_s2 = bin2dec(lsf_raw_bits(37:42)) + 1;
        
        p.lsf_indices.base = 0; % 容错
    end
end
%=====================================================================================================================
...
function lsf_out = decode_lsf_sc1200(p, lsf_prev, cb)
% =========================================================================
% LSF Decoder (Bit-Exact Logic)
% 核心修正: 
%   1. 修正字段名引用: p.lsf -> p.lsf_indices
%   2. 区分 Mixed (9-bit Base) 和 VVV (24-bit Base)
% =========================================================================
    lsf_out = zeros(10, 3);
    mean_lsf = cb.msvq_mean(:);
    
    % [关键修正] 这里必须读取 lsf_indices，因为 unpack 函数输出的就是这个名字
    idx = p.lsf_indices; 
    
    % 检查是否处于 Mode 1 (插值模式)
    % 注意: unpack 函数里根据 UV 设定了 p.mode
    if isfield(p, 'mode') && p.mode == 1
        % --- Mode 1: 插值模式 ---
        
        if isfield(p, 'is_case_001') && p.is_case_001
            % === Case A: Mixed Mode (001) - 9-bit Base ===
            lsf_base = cb.lsp_uv_9(:, idx.base_idx);
            
            % 4级残差 (Q17)
            res = cb.res_cb{1}(:, idx.res_s1) + ...
                  cb.res_cb{2}(:, idx.res_s2) + ...
                  cb.res_cb{3}(:, idx.res_s3) + ...
                  cb.res_cb{4}(:, idx.res_s4);
        else
            % === Case B: VVV Mode - 24-bit Base ===
            % 4级 Base MSVQ
            base_quant = cb.lsp_v_cb{1}(:, idx.base_s1) + ...
                         cb.lsp_v_cb{2}(:, idx.base_s2) + ...
                         cb.lsp_v_cb{3}(:, idx.base_s3) + ...
                         cb.lsp_v_cb{4}(:, idx.base_s4);
                         
            lsf_base = mean_lsf + base_quant;
        ...
        % --- 公共预测与重组逻辑 ---
        w_vec = cb.inpCoef(:, idx.int_idx); % Q14 -> Float
        w_f2 = w_vec(1:10);
        w_f3 = w_vec(11:20);
        ...
        lsf_out = [lsf_f1, lsf_f2, lsf_f3];
        
    else
        % --- Mode 0: 独立模式 (UV) ---
        % Frame 1
        lsf_out(:, 1) = mean_lsf + cb.lsp_v_cb{1}(:, idx.f1_s1) + cb.lsp_v_cb{2}(:, idx.f1_s2);
        % Frame 2
        lsf_out(:, 2) = mean_lsf + cb.lsp_v_cb{1}(:, idx.f2_s1) + cb.lsp_v_cb{2}(:, idx.f2_s2);
        % Frame 3
        lsf_out(:, 3) = mean_lsf + cb.lsp_v_cb{1}(:, idx.f3_s1) + cb.lsp_v_cb{2}(:, idx.f3_s2);
    end
end
%================================================================================================================================
...
function [pitch_vec, uv_flags, recovered_idx] = decode_pitch_vq_1200(uv_encoded, pitch_9bit, cb)
% =========================================================================
% Pitch Decoder (Bit-Exact Logic - C Standard Aligned)
% Input: 
%   uv_encoded: 3-bit integer (0-7)
%   pitch_9bit: 9-bit index
% Output:
%   pitch_vec:  3x1 Linear Pitch Values
%   uv_flags:   3x1 Flags (1=Voiced, 0=Unvoiced)
% =========================================================================

    pitch_vec = zeros(3, 1);
    
    % --- 1. UV 模式解析 (依据 C 代码 pitch_uvflag_map 逻辑) ---
    is_vvv = false;
    high_bits = 0;
    
    if pitch_9bit > 400 && uv_encoded == 0
         uv_encoded = 3; % 强行切到 VVV 模式试试
    end

    switch uv_encoded
        % === VVV 模式 (携带 Pitch 高位) ===
        case 3 % 011 -> VVV, High=00 (0)
            uv_flags = [1; 1; 1]; is_vvv = true; high_bits = 0;
        case 5 % 101 -> VVV, High=01 (1)
            uv_flags = [1; 1; 1]; is_vvv = true; high_bits = 1;
        case 6 % 110 -> VVV, High=10 (2)
            uv_flags = [1; 1; 1]; is_vvv = true; high_bits = 2;
        case 7 % 111 -> VVV, High=11 (3)
            uv_flags = [1; 1; 1]; is_vvv = true; high_bits = 3;
            
        % === 混合模式 (Mixed) ===
        case 1 % 001 -> VVU (注意 C 代码里的 uv_config=1 对应 VVU)
            uv_flags = [1; 1; 0];
        case 2 % 010 -> VUV
            uv_flags = [1; 0; 1];
        case 4 % 100 -> UVV
    ...
    if is_vvv
        % VVV 模式: 拼接 11-bit 索引
        % Index = (HighBits << 9) | LowBits
        recovered_idx = high_bits * 512 + pitch_9bit;
        
        % 查 VVV 码本
        idx_mat = recovered_idx + 1; % MATLAB 1-based
        if idx_mat > size(cb.pitch_vq_cb_vvv, 2)
            idx_mat = 1; % 越界保护
        end
        log_pitch = cb.pitch_vq_cb_vvv(:, idx_mat);
        
    else
        % 非 VVV 模式: 直接用 9-bit 查 UVV 码本
        recovered_idx = pitch_9bit;
        
        % 如果 UV 是 [0 0 0] (UUU)，理论上 pitch 应该是默认值
        % 但 C 代码有时也会传个随机 pitch，这里我们照常查表
        % 在上层逻辑里会被 uv_flags 屏蔽掉
        idx_mat = recovered_idx + 1;
        if idx_mat > size(cb.pitch_vq_cb_uvv, 2)
            idx_mat = 1;
        end
        log_pitch = cb.pitch_vq_cb_uvv(:, idx_mat);
    end
    
    % --- 3. Log -> Linear ---
    pitch_vec = 10.^log_pitch;
end
