...
% --- [修正] 强制植入 20维残差码本 ---
fprintf('正在将 res256_64_64_64.txt 植入 codebooks 结构体...\n');
...
% 核心：必须存入 codebooks.res_cb 字段！
codebooks.res_cb = cell(1,4);
codebooks.res_cb{1} = RES_FULL(:, 1:256);
codebooks.res_cb{2} = RES_FULL(:, 257:320);
codebooks.res_cb{3} = RES_FULL(:, 321:384);
codebooks.res_cb{4} = RES_FULL(:, 385:448);
fprintf('✅ res_cb 字段已成功注入。\n');
fprintf(' 完成。\n');
...
% ✅ 新的调用 (更简单、更稳健)：
% 注意：我还没让你算 bpvc 向量，所以这里暂时传 []，让函数内部自己处理
    [p_idx_c, bpvc_idx_c, jit_idx_c] = quantize_pitch_bpvc_c_style(sf_pitches, sf_v_dec, []);
...
% 提示：uv_bits_c 不需要了，因为 packer 会自己生成。
    ...
    % === 🔍 间谍代码：查看数值范围 ===
     ...
     fprintf('  -> 码本 (9bit) 前5个值: %.4f %.4f %.4f %.4f %.4f\n', codebooks.lsf_cb_9bit(1:5, 1));
     fprintf('  -> 码本最大值: %.4f\n', max(codebooks.lsf_cb_9bit(:)));
% ================================
...
% =====================================================================
    ...
    % 准备参数
    p_idx = superframe_pitch_idx_c(s_idx); % Pitch
    g_idx = superframe_gain_indices(s_idx); % Gain
    uv_flags = sf_v_dec; % UV [1x3]
